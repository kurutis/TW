...
const CartModal = ({ isOpen, onClose }) => {
  ...
  // Обработка разлогина при истечении сессии
  useEffect(() => {
    if (shouldLogout) {
      dispatch(logout());
      dispatch(showAuthModal({
        message: 'Сессия истекла. Пожалуйста, войдите снова',
        callback: () => dispatch(fetchCart())
      }));
      dispatch(resetError());
    }
  ...
  useEffect(() => {
    if (isOpen) {
      if (isAuthenticated) {
        dispatch(fetchCart()).catch(console.error);
      } else {
        dispatch(showAuthModal({
    ...
    if (!isAuthenticated || localLoading) return;
        ...
        {isLoading ? (
          ...
          <>
            <div className={s.itemsList}>
              {items.map(item => (
                <div key={`${item.id}-${item.selectedColor || 'default'}`} className={s.cartItem}>
                  <img 
                  ...
                  <div className={s.itemDetails}>
                    ...
                    <div className={s.quantityControls}>
                      ...
                      <button
                        onClick={() => handleQuantityChange(item.id, item.quantity + 1)}
                        disabled={item.quantity >= (item.stock || 99) || localLoading}
                      >
                        +
                      </button>
                    </div>
            ...
            <div className={s.footer}>
              <div className={s.total}>Итого: {total} ₽</div>
