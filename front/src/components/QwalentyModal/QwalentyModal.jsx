  ...
  try {
    ...
    if (error?.status === 401) {
      ...
      dispatch(showAuthModal({
        message: error.message || 'Для добавления в корзину требуется авторизация',
        callback: { type: 'RETRY_ADD_TO_CART' }
      }));
    } else {
    ...
    useEffect(() => {
        if (colors.length > 0) {
            setSelectedColor(colors[selectedColorIndex] || colors[0]);
        }
        ...
        <div className={s.modalOverlay} onClick={onClose}>
            <div className={s.modalContent} onClick={e => e.stopPropagation()}>
                ...
                >
                    ...
                    {!isAdding && <img src={basket} alt="" className={s.basketIcon} />}
                </button>
            </div>
        </div>
    );
