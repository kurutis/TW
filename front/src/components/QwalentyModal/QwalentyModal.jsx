...
export const QuantityModal = ({ 
  ...
  const [selectedColor, setSelectedColor] = useState(colors[initialColorIndex] || null);
  ...
  useEffect(() => {
    if (colors.length > 0) {
      const initialColor = colors[initialColorIndex] || colors[0];
      setSelectedColor(initialColor);
    }
  }, [colors, initialColorIndex, onColorSelect]);
    ...
    <div className={s.modalOverlay} onClick={onClose}>
      <div className={s.modalContent} onClick={e => e.stopPropagation()}>
          ...
          <>
            ...
            <div className={s.colors}>
              {colors.map((color, index) => (
                <button
                  key={index}
                  className={`${s.color_btn} ${selectedColor === color ? s.active : ''}`}
        ...
        <button 
          ...
          onClick={handleAdd}
          disabled={(colors.length > 0 && !selectedColor) || isAdding}
        >
          {isAdding ? 'Добавление...' : (
            <>
              Добавить в корзину
              <img src={basketIcon} alt="" className={s.basketIcon} />
            </>
          )}
        </button>
      </div>
